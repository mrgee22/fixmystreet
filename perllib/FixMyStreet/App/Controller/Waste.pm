...
sub garden_check : Chained('garden_setup') : Args(0) {
    ...
    # TODO Sacks
    my $bin_service = $c->stash->{services}{$c->cobrand->garden_waste_service_id};
    if (!$bin_service) {
        # If no subscription, go to /garden
        $uri .= '/garden';
    } elsif ($bin_service->{garden_due} || $bin_service->{garden_overdue}) {
        # If needing renewal
        $uri .= '/garden_renew';
    } else {
    }
